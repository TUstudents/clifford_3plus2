...
## Combined Route 1 + Route 2

The direct synthesis is implemented in the same CLI:

```bash
uv run python scripts/spatial_1d_alpha_search.py --variant combined --check
```

It composes the Route-1 on-site noncommuting rule with the Route-2 winding
layer:
...
onsite_lower_rank_central_idempotents = 0
...
This is the cleanest composition of the two partial mechanisms so far. The
on-site rule supplies the coarse `[0,4,6,10]` center and a finite four-element
compatible-`J` set; the spatial winding couples those four signs down to the
global `±J` pair. The remaining strict failure is also precise: neither of
the two transported `J` choices lies in the joint rule algebra generated by
`U1`, `U2`, and the combined hopping coefficients.

So the synthesis reaches the topological `±J` shape but not the rule-generated
`J` standard. The next microscopic input must make the transported orientation
rule-local, not only topologically admissible.
