...
def _memory_budget_summary(
    *,
    ...
    max_sites = int(usable_bytes // bytes_per_site)
    return {
...
def _benchmark_payload(
    *,
    ...
    def state_rollout_kernel(local_qca_state):
        return sm_run_qca_state_rollout(local_qca_state, config, steps=steps)

    if rollout_output == "state":
        initial_arg = prepared
        jitted_rollout = jax.jit(state_rollout_kernel, donate_argnums=(0,) if donate_state else ())
    else:
        initial_arg = state
...
def _benchmark_sweep_payload(
    *,
    ...
    bytes_per_site = float(benchmarks[0]["memory"]["complex64_bytes_per_site"])
    return {
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default="hop_sum",
        help="Use the fast eight-hop stream or the lower-temp-memory split-axis stream.",
    )
    parser.add_argument(
        "--rollout-output",
        choices=("diagnostic", "state"),
        default="diagnostic",
        help="Compile the diagnostic rollout or a pure final-state rollout without observable reductions.",
    )
