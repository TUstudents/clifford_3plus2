"""Physical-right production tick for QCA_SMv0.

Stage 21 merges the local three-family Yukawa/Higgs production source into the
Stage 20 physical-right sourced gauge tick.  The production ordering remains
the Stage 15 symmetric local half-collision / BCC transport / half-collision
rule, but the gauge-source spine and the transport kernel are the
physical-right bridged ones.

The local Yukawa/Higgs source is still the explicit simulator Yukawa input
built in Stages 7/10/15.  This stage rewrites the tick carrier; it does not
derive Yukawa matrices, Higgs parameters, boundary rules, or quantized
registers.
"""
...
class PhysicalRightProductionSMTickDiagnostics(NamedTuple):
    """Focused diagnostics for Stage 21 physical-right production tick."""
    ...
    transport_production_difference_norm: jnp.ndarray
    physical_source_force_difference_norm: jnp.ndarray
...
def sm_physical_right_production_higgs_force(
    state: jnp.ndarray,
    higgs: jnp.ndarray,
    ...
    *,
    ...
    """Return Higgs dynamics plus local Yukawa source in the Stage 21 tick."""
...
def sm_apply_physical_right_production_higgs_momentum_kick(
    ...
    *,
    ...
    """Kick Higgs momenta by the Stage 21 production Higgs source."""

    lattice_shape = _validate_family_state(state)
    _validate_higgs_momenta(higgs_momenta, lattice_shape)
    source = sm_physical_right_production_higgs_force(
        state,
        higgs,
        higgs_links,
        parameters=parameters,
...
def sm_physical_right_production_sm_tick(
    state: jnp.ndarray,
    higgs: jnp.ndarray,
    higgs_momenta: jnp.ndarray,
    ...
    *,
    ...
    """Advance one physical-right production tick.
...
def sm_physical_right_production_tick_diagnostics() -> PhysicalRightProductionSMTickDiagnostics:
    """Return focused Stage 21 physical-right production diagnostics."""
    ...
    stage20_zero = sm_physical_right_sourced_sm_tick(
        state,
        higgs,
        higgs_momenta,
        sm_links,
    ...
    production_zero = sm_physical_right_production_sm_tick(
        state,
        higgs,
        higgs_momenta,
        sm_links,
    ...
    production = sm_physical_right_production_sm_tick(
        state,
        higgs,
        higgs_momenta,
        sm_links,
    ...
    stage20_default = sm_physical_right_sourced_sm_tick(
        state,
        higgs,
        higgs_momenta,
        sm_links,
    ...
    transport_production = sm_family_production_sm_tick(
        state,
        higgs,
        higgs_momenta,
        sm_links,
    ...
    kicked = sm_apply_physical_right_production_higgs_momentum_kick(
        higgs_momenta,
        state,
        higgs,
        higgs_links,
    ...
    restored = sm_apply_physical_right_production_higgs_momentum_kick(
        kicked,
        state,
        higgs,
        higgs_links,
    ...
    return PhysicalRightProductionSMTickDiagnostics(
        zero_yukawa_state_reduction_residual=jnp.max(jnp.abs(production_zero[0] - stage20_zero[0])),
        zero_yukawa_higgs_reduction_residual=jnp.max(jnp.abs(production_zero[1] - stage20_zero[1])),
        zero_yukawa_higgs_momentum_reduction_residual=jnp.max(jnp.abs(production_zero[2] - stage20_zero[2])),
        zero_yukawa_sm_link_reduction_residual=jnp.max(jnp.abs(production_zero[3] - stage20_zero[3])),
        zero_yukawa_sm_momentum_reduction_residual=jnp.max(jnp.abs(production_zero[4] - stage20_zero[4])),
        zero_yukawa_higgs_link_reduction_residual=jnp.max(jnp.abs(production_zero[5] - stage20_zero[5])),
        zero_yukawa_source_norm=jnp.linalg.norm(zero_source),
        nonzero_yukawa_source_norm=jnp.linalg.norm(source),
        production_state_delta_norm=jnp.linalg.norm(production[0] - stage20_default[0]),
        production_higgs_momentum_delta_norm=jnp.linalg.norm(production[2] - stage20_default[2]),
        transport_production_difference_norm=jnp.linalg.norm(production[0] - transport_production[0]),
        physical_source_force_difference_norm=jnp.linalg.norm(
            sm_physical_right_sourced_link_force(state, higgs, sm_links, higgs_links)
            - sm_physical_right_sourced_link_force(zero_state, vacuum, sm_links, higgs_links),
        ),
