"""Tests for QCA_SMv0 physical-right production tick."""
...
from clifford_3plus2_d5.qca_smv0.sm_physical_right_sourced_tick import sm_physical_right_sourced_sm_tick
from clifford_3plus2_d5.sim.state import state_norm_squared


def _stage21_fields(lattice_shape: tuple[int, int, int] = (1, 1, 1)):
...
def test_zero_yukawas_reduce_physical_right_production_tick_to_stage20_tick() -> None:
    state, higgs, higgs_momenta, sm_links, sm_momenta, higgs_links = _stage21_fields()
    ...
    actual = sm_physical_right_production_sm_tick(
        state,
        higgs,
        higgs_momenta,
        sm_links,
...
def test_physical_right_production_higgs_force_adds_yukawa_source() -> None:
    state, higgs, _, _, _, higgs_links = _stage21_fields()

    production_force = sm_physical_right_production_higgs_force(state, higgs, higgs_links)
...
def test_physical_right_zero_state_vacuum_has_zero_yukawa_source() -> None:
    state, _, _, _, _, higgs_links = _stage21_fields()
    ...
    assert jnp.linalg.norm(sm_physical_right_production_higgs_force(zero_state, vacuum, higgs_links)) > 1e-6


def test_physical_right_production_higgs_momentum_kick_is_reversible_for_frozen_fields() -> None:
    state, higgs, higgs_momenta, _, _, higgs_links = _stage21_fields()
...
def test_physical_right_production_tick_preserves_norm_and_updates_local_yukawa_sector() -> None:
    state, higgs, higgs_momenta, sm_links, sm_momenta, higgs_links = _stage21_fields()
    ...
    stage20 = sm_physical_right_sourced_sm_tick(
        state,
        higgs,
        higgs_momenta,
        sm_links,
    ...
    assert jnp.linalg.norm(production[0] - stage20[0]) > 1e-6
    assert jnp.linalg.norm(production[2] - stage20[2]) > 1e-6
    assert sm_link_unitarity_residual(production[3]) < 7e-7
    assert sm_higgs_link_unitarity_residual(production[5]) < 7e-7


def test_physical_right_production_tick_differs_from_transport_production_tick() -> None:
    state, higgs, higgs_momenta, sm_links, sm_momenta, higgs_links = _stage21_fields()

    physical = sm_physical_right_production_sm_tick(
        state,
        higgs,
        higgs_momenta,
        sm_links,
    ...
    transport = sm_family_production_sm_tick(
        state,
        higgs,
        higgs_momenta,
        sm_links,
    ...
    assert jnp.linalg.norm(physical[0] - transport[0]) > 1e-3


def test_physical_right_production_tick_diagnostics_and_jit_pass_stage_thresholds() -> None:
    ...
    assert diagnostics.transport_production_difference_norm > 1e-3
    assert diagnostics.physical_source_force_difference_norm > 1e-2
    ...
    assert diagnostics.jit_delta_family_state < 5e-7
...
def test_physical_right_production_tick_is_jittable() -> None:
    state, higgs, higgs_momenta, sm_links, sm_momenta, higgs_links = _stage21_fields()
