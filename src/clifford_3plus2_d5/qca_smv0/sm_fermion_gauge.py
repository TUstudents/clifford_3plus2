"""BCC streaming fermion current for QCA_SMv0.

Stage 11 adds the gauge-link source from the already-implemented BCC Dirac
transport.  The current is the left-trivialized derivative of the local
gauge-invariant streaming bilinear

``E_stream = Re sum_x,h psi[x]^dag D_h U_h[x] psi[x+h]``.

This is a simulator-level backreaction rule for the fermion current on gauge
links.  It does not yet combine the Higgs and fermion sources into a full
production update, and it does not introduce boundary or flavor derivations.
"""
...
class FermionGaugeCurrentDiagnostics(NamedTuple):
    """Focused diagnostics for Stage 11 BCC streaming fermion current."""
    ...
    gauss_zero_state_residual: jnp.ndarray
...
def sm_dirac_streaming_local_density(state: jnp.ndarray, links: jnp.ndarray) -> jnp.ndarray:
    """Return local ``Re psi[x]^dag sum_h D_h U_h psi[x+h]`` density."""
    ...
    for index, displacement in enumerate(BCC_DISPLACEMENTS):
        ...
        hopped = jnp.einsum("rs,...sd->...rd", hops[index], linked)
        local = local + jnp.sum(jnp.conj(state) * hopped, axis=(-2, -1))
    return jnp.real(local)


def sm_dirac_streaming_energy_density(state: jnp.ndarray, links: jnp.ndarray) -> jnp.ndarray:
    """Return mean BCC Dirac streaming bilinear density."""

    return jnp.mean(sm_dirac_streaming_local_density(state, links))
...
def sm_fermion_left_gauge_current(state: jnp.ndarray, links: jnp.ndarray, *, epsilon: float = 3e-2) -> jnp.ndarray:
    """Return the fermion streaming derivative with respect to left link updates."""
    ...
    def energy_for_index(index: jnp.ndarray, updates: jnp.ndarray) -> jnp.ndarray:
        ...
        return sm_dirac_streaming_energy_density(state, perturbed_links)
...
def sm_streaming_fermion_charge_density(state: jnp.ndarray) -> jnp.ndarray:
    """Return local gauge charge density from the SM Dirac state."""

    _validate_sm_state(state)
    generators = sm_generators(dtype=state.dtype)
    return jnp.imag(jnp.einsum("...si,aij,...sj->...a", jnp.conj(state), generators, state))


def sm_fermion_gauss_constraint(links: jnp.ndarray, momenta: jnp.ndarray, state: jnp.ndarray) -> jnp.ndarray:
    """Return electric divergence minus the streaming fermion charge density."""

    return sm_electric_divergence(links, momenta) - sm_streaming_fermion_charge_density(state)


def sm_apply_fermion_gauge_momentum_kick(
    momenta: jnp.ndarray,
    state: jnp.ndarray,
    links: jnp.ndarray,
    *,
    ...
    """Kick SM link momenta by the BCC streaming fermion current."""

    _validate_sm_momenta(momenta, links)
    current = sm_fermion_left_gauge_current(state, links, epsilon=epsilon)
    return momenta - jnp.asarray(step_size, dtype=momenta.dtype) * current


def sm_fermion_gauge_kick_then_transport(
    state: jnp.ndarray,
    links: jnp.ndarray,
    momenta: jnp.ndarray,
    *,
    ...
    """Kick momenta by the fermion current, update links, then transport state."""

    kicked_momenta = sm_apply_fermion_gauge_momentum_kick(
        momenta,
        state,
        links,
        step_size=step_size,
...
def sm_fermion_gauge_current_diagnostics() -> FermionGaugeCurrentDiagnostics:
    """Return focused Stage 11 BCC streaming fermion-current diagnostics."""
    ...
    step_size = 0.005
    kicked = sm_apply_fermion_gauge_momentum_kick(momenta, state, links, step_size=step_size)
    restored = sm_apply_fermion_gauge_momentum_kick(kicked, state, links, step_size=-step_size)
    updated_state, updated_links, _ = sm_fermion_gauge_kick_then_transport(
        state,
        links,
        momenta,
        step_size=step_size,
    ...
    jit_kick = jitted_kick(momenta, state, links, step_size=step_size)

    return FermionGaugeCurrentDiagnostics(
        streaming_energy_reality_residual=jnp.abs(jnp.imag(energy + 0j)),
        zero_state_current_norm=jnp.linalg.norm(sm_fermion_left_gauge_current(zero_state, links)),
        ...
        gauss_zero_state_residual=jnp.linalg.norm(sm_fermion_gauss_constraint(links, jnp.zeros_like(momenta), zero_state)),
