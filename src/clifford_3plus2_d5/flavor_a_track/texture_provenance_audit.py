...
def texture_provenance_verdict(
    structure_derived: bool, free_enumerated: bool, predictive: bool
) -> str:
    """Return the combined A3b verdict from the three sub-gate flags.

    Pure decision so every branch (PASS and both KILLs) is unit-testable:
    - full pass needs derived structure (B1), a standing free-input
      enumeration (B2), and a predictive count (B3);
    - a non-predictive count is numerology;
    - otherwise a provenance failure (B1 factor not derived, or B2 enumeration
      does not stand).
    """

    if structure_derived and free_enumerated and predictive:
        return "TEXTURE_STRUCTURE_DERIVED_HIERARCHY_INPUT"
    if not predictive:
        return "TEXTURE_NUMEROLOGY_KILL"
    return "TEXTURE_PROVENANCE_KILL"
...
def texture_provenance_audit_payload() -> TextureProvenanceAuditPayload:
    ...
    free_enumerated = b2.final_verdict == "FREE_INPUTS_ENUMERATED"
    predictive = b3.final_verdict == "TEXTURE_PREDICTIVE"

    final_verdict = texture_provenance_verdict(
        structure_derived, free_enumerated, predictive
    )

    if final_verdict == "TEXTURE_STRUCTURE_DERIVED_HIERARCHY_INPUT":
        interpretation = (
    ...
    elif final_verdict == "TEXTURE_NUMEROLOGY_KILL":
        interpretation = (
        ...
        interpretation = (
            f"A provenance check failed (B1={b1.final_verdict}, "
            f"B2={b2.final_verdict}): a claimed-derived factor is not actually "
            "derived, or the free-input enumeration does not stand."
        )
