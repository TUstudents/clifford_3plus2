...
def derived_factors_verdict(all_checks_pass: bool) -> str:
    """Return the B1 verdict for a given all-checks-pass flag.

    Pure decision so the KILL branch is unit-testable without re-running the
    symbolic checks (a single failed factor, e.g. an external
    LEPTONIC_PHASE_WORD_DERIVED_KILL, propagates here).
    """

    if all_checks_pass:
        return "DERIVED_FACTORS_CATALOGUED"
    return "DERIVED_FACTORS_CATALOGUED_KILL"
...
def derived_factors_audit_payload() -> DerivedFactorsAuditPayload:
    ...
    final_verdict = derived_factors_verdict(all_pass)
