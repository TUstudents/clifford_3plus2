...
# The texture program declares exactly these free inputs (B3 counts against them).
N_FREE_INPUTS = 4


def free_inputs_verdict(even_additive: bool, n_free: int) -> str:
    """Return the B2 verdict for a given (even_additive, n_free).

    The enumeration only stands if the assigned depths have the claimed
    even+additive structure (otherwise the CKM additivity prediction is void)
    and the free-input count is the declared four. Pure decision so the KILL
    branch is unit-testable.
    """

    if even_additive and n_free == N_FREE_INPUTS:
        return "FREE_INPUTS_ENUMERATED"
    return "FREE_INPUTS_NOT_ENUMERATED_KILL"
...
def free_inputs_audit_payload() -> FreeInputsAuditPayload:
    ...
    final_verdict = free_inputs_verdict(even_additive, len(inputs))

    if final_verdict == "FREE_INPUTS_ENUMERATED":
        interpretation = (
            f"The texture program has {len(inputs)} free inputs: the quark depth "
            "embedding {0,2,6} (fit to the CKM hierarchy; even+additive checked), "
            "the charged-lepton two-step depth, the flat-coin r=1 ergodicity prior "
        ...
        interpretation = (
            f"The free-input enumeration does not stand: even+additive depths = "
            f"{even_additive}, free-input count = {len(inputs)} (expected "
            f"{N_FREE_INPUTS}). The assigned depths lack the structure the CKM "
            "additivity prediction relies on, or the input set is mis-counted."
        )
