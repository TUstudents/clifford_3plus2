...
def coupling_structure_verdict(
    casimir_ok: bool,
    color_factor_ok: bool,
    bcc_ok: bool,
    coin_ok: bool,
    split_ok: bool,
) -> str:
    """Return the A3-3 verdict from the five checks. Pure decision (KILL-testable)."""

    if casimir_ok and color_factor_ok and bcc_ok and coin_ok and split_ok:
        return "SECTOR_STRUCTURE_IN_COUPLINGS"
    return "SECTOR_STRUCTURE_IN_COUPLINGS_KILL"
...
def coupling_structure_audit_payload() -> CouplingStructureAuditPayload:
    ...
    final_verdict = coupling_structure_verdict(casimir_ok, cf_ok, bcc_ok, coin_ok, split_ok)

    if final_verdict == "SECTOR_STRUCTURE_IN_COUPLINGS":
        interpretation = (
