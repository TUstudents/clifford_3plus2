...
from clifford_3plus2_d5.flavor_a_track.texture_provenance_audit import (
    REMAINING_DECLARED_INPUTS,
    texture_provenance_audit_payload,
    texture_provenance_verdict,
)
...
def test_combined_verdict_kills_on_every_failure_mode() -> None:
    # Decisive negative control for the aggregate. B2 now participates: a B2
    # failure (free-input enumeration does not stand) routes to a provenance
    # kill, alongside a B1 structure failure; a non-predictive count is a
    # numerology kill.
    assert (
        texture_provenance_verdict(True, True, True)
        == "TEXTURE_STRUCTURE_DERIVED_HIERARCHY_INPUT"
    )
    assert texture_provenance_verdict(False, True, True) == "TEXTURE_PROVENANCE_KILL"
    assert texture_provenance_verdict(True, False, True) == "TEXTURE_PROVENANCE_KILL"
    assert texture_provenance_verdict(True, True, False) == "TEXTURE_NUMEROLOGY_KILL"
