...
def color_label_partition_verdict(
    cores_match: bool,
    color_split_is_three_three: bool,
    partition_complete: bool,
    core_plus_color: bool,
) -> str:
    """Return the U2 verdict from the four checks. Pure decision (KILL-testable)."""

    if cores_match and color_split_is_three_three and partition_complete and core_plus_color:
        return "SECTOR_DIFFERENCE_IS_COLOR_LABEL"
    return "SHELLS_INDEPENDENT_KILL"
...
def color_label_partition_audit_payload() -> ColorLabelPartitionAuditPayload:
    ...
    final_verdict = color_label_partition_verdict(
        cores_match, color_split == (3, 3), partition_complete, core_plus_color
    )

    if final_verdict == "SECTOR_DIFFERENCE_IS_COLOR_LABEL":
        interpretation = (
