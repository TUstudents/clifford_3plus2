...
Every verdict records an `exact_working_field`. For the generic checker this is
the field generated by the exact SymPy entries supplied by the rule layers. The
Floquet-α and Defect-β certificates specialize that to `QQ(zeta_12)`, with the
real matrices living in `QQ(sqrt(3))`.

The checker records `compatible_centralizer_dimension` before attempting the
nonlinear `J` solve. It solves the nonlinear `J` equations exactly only when
the generated algebra or compatible centralizer basis is below the configured
solve bound. When the compatible centralizer is too large, it reports
`compatible_j_solved: false` and refuses to mark `J` forced.
