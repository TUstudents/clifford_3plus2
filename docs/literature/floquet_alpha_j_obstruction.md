...
## Headline: The Centralizer Gap

The active obstruction has a name: the centralizer gap.

Let `A_R` be the algebra generated by the mandatory rule layers, let
`C_R = Z(A_R)` be its rule-local center, and let `Cent_R` be the joint
centralizer of the rule layers in `M_10(R)`. Since the rule layers generate
`A_R`,

```text
A_R ∩ Cent_R = Z(A_R) = C_R.
```

For the noncommuting signed-twist representative, the checker reports:
...
The four compatible `J` candidates are finite points in `Cent_R` solving
`J^2 = -I` and `J^T J = I`, but none lies in the rule's own algebra or in the
rule-local center. This 3-dimensional quotient direction is the missing
structure. Strict forcedness now requires a microscopic primitive that puts an
...
## Time-Reversal Sidecar

Route 3 adds a declared real involution `K`:
...
So declared time reversal reduces the continuous compatible-`J` freedom, but
it does not collapse it to global `±J`, and the involution is not generated by
the Floquet rule algebra. It remains a sidecar constraint until a microscopic
rule produces `K`.
