...
projectors. Raising the closure cap to `48` shows this is a structured family:
the first six sampled candidates all close at algebra dimension `34`, have
center dimension `4`, central idempotent ranks `[0,4,6,10]`, and compatible
centralizer dimension `4`. This is the first unseeded Path-A coarse center
hit. The split-center `J` calculation is negative for all six candidates:
both generated and compatible `J` counts are `0`, so this monomial-hop family
does not carry the microscopic complex structure.

Proposition 4a (projector-free monomial-hop Bloch `J` obstruction, finite
sampled witness). Let `T(z)` range over the first six projector-free Path-A
monomial-hop candidates generated by `scripts/bloch_path_a_stepwise.py` with
`max_candidates = 6`, using sampled period `12` and joint algebra
`A = R<T(zeta^0),...,T(zeta^11)>`. Then each checked candidate satisfies:

1. `dim A = 34`;
2. `Z(A)` has central idempotent ranks `[0,4,6,10]`;
3. `Cent({T(zeta^j)}_j)` has dimension `4`;
4. the split-center solver finds no real orthogonal `J` in either `Z(A)` or
   the compatible centralizer;
5. the projected compatible centralizers are split real: the rank-6 block is
   `R^3`, and the rank-4 block is `R`.

Consequently the checked projector-free monomial-hop family forces the coarse
center but cannot satisfy the strict bridge, because the compatible `J` set is
empty. This differs from the on-site Route-1 obstruction, where compatible
`J` candidates existed but lived outside the rule algebra.
...
computed from the algebra multiplication table, central idempotents are solved
exactly, and the compatible centralizer is the nullspace of the exact
commutator equations with all sampled symbols. The bounded split-center `J`
solver uses the rank-`(6,4)` idempotent pair: on each projected two-dimensional
summand it solves `(uP + vx)^2 = -P` analytically via
`u = -bv/2`, `v^2 = -1/(a + b^2/4)`, then filters candidates by realness and
orthogonality. The command
`uv run python scripts/bloch_path_a_stepwise.py --max-candidates 6 --max-algebra-dim 48 --center-top 6 --idempotents --centralizer --j-solve --projected-centralizer --jobs 4 --check`
certifies the numbered outputs for all six candidates and explicitly reports
`generated_j_solved = True` and `compatible_j_solved = True` with both
candidate counts equal to `0`. The projected-centralizer diagnostic computes
the multiplication table on each coarse block, solves its primitive idempotent
decomposition, and reports
`rank6:split_real/dim3/components['R', 'R', 'R']` and
`rank4:split_real/dim1/components['R']`. Thus the negative `J` result is
...
That argument is not yet a proof of the implemented Path-A class. The actual
projector-free monomial-hop coefficients are `U_local E_s`, where `U_local` is
the Route-1 noncommuting on-site update and `E_s` is a mode-edge matrix. The
mode cycles may cross the eventual coarse blocks, and `P_alpha/P_eta` are
outputs of the joint sampled algebra rather than inputs available before the
proof. Therefore one cannot assume, without an additional invariant, that the
projected algebra `P A P` is generated only by block-scalar root-of-unity
phases.
...
effective rule to derive from microscopic hopping layers. A serious unseeded
Route-2 search must include finite-order on-site primitives, noncommuting
orientation twists, and translation-protected real-orthogonal hopping
generators. Its seed guardrail must also be algebraic: if the coefficient
algebra generates `P_alpha` or `P_eta`, the candidate has hidden the answer
even when no raw coefficient is visibly a diagonal projector.
