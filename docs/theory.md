...
Proposition 4 (projector-free monomial-hop Bloch `J` obstruction, finite
sampled class). Let `T(z)` range over the first six projector-free Path-A
...
`A = R<T(zeta^0),...,T(zeta^11)>`. Then each candidate satisfies:
...
Consequently this projector-free monomial-hop class forces the coarse center
but cannot satisfy the strict bridge, because the compatible `J` set is empty.
This differs from the on-site Route-1 obstruction, where compatible `J`
candidates existed but lived outside the rule algebra.
