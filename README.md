...
| [`qca_smv0`](src/clifford_3plus2_d5/qca_smv0/) | **Stage 4 PASS** | Focused simulator sidecar for the next Standard-Model QCA prototype. Stage 1 implements the free BCC Weyl/Dirac bulk walk in JAX. Stage 2 adds a local 32-component SM carrier and static `SU(3)_c x SU(2)_L x U(1)_Y` BCC gauge links. Stage 3 adds pure dynamic SM gauge fields with link momenta, Wilson force, Hamiltonian leapfrog, electric-divergence/Gauss diagnostics, weak-field Yang-Mills audits, no-backreaction spectator fermion transport, and JIT compatibility. Stage 4 adds a local Higgs doublet and exact site-local Higgs/Yukawa collision with SM door checks, chirality flip, massive dispersion, and JIT compatibility. Boundary, matter backreaction, dynamic Higgs evolution, and flavor recirculation remain future sessions. |
...
- `qca_smv0` is a focused simulator sidecar for the next SM-QCA prototype; it currently implements the free BCC Weyl/Dirac bulk walk, static SM gauge-background transport, pure dynamic SM gauge fields, and a local Higgs/Yukawa collision, but should not be read as a boundary, matter-backreaction, dynamic-Higgs, or flavor claim.
