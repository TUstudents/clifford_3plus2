...
| [`qca_smv0`](src/clifford_3plus2_d5/qca_smv0/) | **Stage 32 PASS** | Focused simulator sidecar for the next Standard-Model QCA prototype. It now includes the free BCC Weyl/Dirac bulk walk, static and dynamic SM gauge transport, Higgs/Yukawa collisions, FN recirculation, center-holonomy CP inputs, three-family production ticks, physical-right bridged transport/current/source updates, rollout/Gauss/energy/variational monitors, refinement and adjoint limitation audits, an explicit inverse helper for the current physical-right production tick, multi-step trajectory reversibility, Loschmidt/tangent echo diagnostics, and a local echo Gram audit. Boundary, quantized registers, microscopic bridge/input derivations, exact energy conservation, and timestep-convergence claims remain future sessions. |
...
- `qca_smv0` is a focused simulator sidecar for the next SM-QCA prototype; it currently implements the BCC/SM/Higgs/FN/production stack through an explicit inverse helper, multi-step reversibility audit, Loschmidt/tangent echo diagnostics, and local echo Gram audit for the current physical-right production tick, but should not be read as a boundary, quantized-register, microscopic bridge, Gauss-projection, exact full-energy conservation, timestep-refined energy convergence, energy-convergent reversible-integrator, or flavor/Higgs-parameter derivation claim.
