...
| [`qca_smv0`](src/clifford_3plus2_d5/qca_smv0/) | **Stage 10 PASS** | Focused simulator sidecar for the next Standard-Model QCA prototype. Stage 1 implements the free BCC Weyl/Dirac bulk walk in JAX. Stage 2 adds a local 32-component SM carrier and static `SU(3)_c x SU(2)_L x U(1)_Y` BCC gauge links. Stage 3 adds pure dynamic SM gauge fields with link momenta, Wilson force, Hamiltonian leapfrog, electric-divergence/Gauss diagnostics, weak-field Yang-Mills audits, no-backreaction spectator fermion transport, and JIT compatibility. Stage 4 adds a local Higgs doublet and exact site-local Higgs/Yukawa collision with SM door checks, chirality flip, massive dispersion, and JIT compatibility. Stage 5 adds finite hidden recirculation paths for FN Yukawa matrices, with `lambda^n` generated by local beam-splitter chains and CKM-like left-frame mismatch read from the generated matrices. Stage 6 adds center-holonomy CP coefficients. Stage 7 embeds the generated three-family Yukawa matrices into an exact local Higgs/Yukawa collision on `(4,32,3)` fermion states. Stage 8 adds dynamic Higgs-field evolution. Stage 9 adds Higgs gauge-current backreaction on electroweak link momenta. Stage 10 adds local Yukawa fermion backreaction into Higgs momenta using `E_Y=Re psi^dag beta Y(H) psi`, with explicit Yukawa-door gauge convention, source covariance, reversible kick, collision-plus-kick, and JIT audits. Boundary, BCC streaming fermion gauge currents, quantized scalar registers, and full sourced SM gauge evolution remain future sessions. |
...
- `qca_smv0` is a focused simulator sidecar for the next SM-QCA prototype; it currently implements the free BCC Weyl/Dirac bulk walk, static SM gauge-background transport, pure dynamic SM gauge fields, one-generation and three-family local Higgs/Yukawa collisions, finite-path FN recirculation, center-holonomy CP coefficients, dynamic classical Higgs-field evolution, Higgs gauge-current backreaction, and local Yukawa fermion backreaction into Higgs momenta, but should not be read as a boundary, BCC streaming fermion-current, quantized-scalar, full sourced SM gauge-evolution, or flavor/Higgs-parameter derivation claim.
