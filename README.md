...
| [`spacetime_qca`](src/clifford_3plus2_d5/spacetime_qca/) | **In progress** | 3D BCC Weyl walk (Bialynicki-Birula 1994), Dirac chiral assembly, hypercubic doubling control, and tensor lift toward the lepton module's internal carrier. Session 20 scaffolding present; full continuum audit not yet complete. |
...
- `lepton` depends on `obstruction_r10.qca.rule_verdict` (VerdictProfile-based verdict path). This is the one cross-module coupling.
- `spacetime_qca` has no cross-module dependencies outside `algebra/`.
