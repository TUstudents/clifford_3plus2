...
- `qca_smv0` is a focused simulator sidecar for the next SM-QCA prototype; it currently implements the free BCC Weyl/Dirac bulk walk, static SM gauge-background transport, pure dynamic SM gauge fields, one-generation and three-family local Higgs/Yukawa collisions, finite-path FN recirculation, center-holonomy CP coefficients, dynamic classical Higgs-field evolution, Higgs gauge-current backreaction, local Yukawa fermion backreaction into Higgs momenta, family-summed BCC streaming currents, family-sourced SM gauge ticks, the full family production tick, a gauge-convention bridge audit, an exact antiunitary singlet bridge, physical-right bridged transport, the physical-right bridged fermion current, a physical-right sourced gauge tick, and a physical-right production tick, but should not be read as a boundary, quantized-register, microscopic bridge, or flavor/Higgs-parameter derivation claim.
