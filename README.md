...
| [`qca_smv0`](src/clifford_3plus2_d5/qca_smv0/) | **Stage 3 PASS** | Focused simulator sidecar for the next Standard-Model QCA prototype. Stage 1 implements the free BCC Weyl/Dirac bulk walk in JAX. Stage 2 adds a local 32-component SM carrier and static `SU(3)_c x SU(2)_L x U(1)_Y` BCC gauge links. Stage 3 adds pure dynamic SM gauge fields with link momenta, Wilson force, Hamiltonian leapfrog, electric-divergence/Gauss diagnostics, weak-field Yang-Mills audits, no-backreaction spectator fermion transport, and JIT compatibility. Boundary, Higgs/Yukawa, matter backreaction, and flavor recirculation remain future sessions. |
...
- `qca_smv0` is a focused simulator sidecar for the next SM-QCA prototype; it currently implements the free BCC Weyl/Dirac bulk walk, static SM gauge-background transport, and pure dynamic SM gauge fields, but should not be read as a boundary, Higgs, matter-backreaction, or flavor claim.
