...
| [`depth_scar`](src/clifford_3plus2_d5/depth_scar/) | **V1–V4 PASS, active** | Boundary repair-scar sidecar for the quark depth spectrum. V1 proves that an `S3 → Z2` path repair scar has `D_scar = 2 Δ(P3)` with spectrum `{0,2,6}`. V2 records the prediction ledger. V3 derives the path scars as the exact minima of a symmetric effective edge-weight potential. V4 proves that CP holonomy requires loop healing: the pure path has no intrinsic phase, while a restored triangle has one gauge-invariant loop phase. |
...
- `depth_scar` upgrades the `{0,2,6}` quark depth spectrum to a graph-Laplacian transfer operator, derives the path scar from an effective edge-weight potential, and identifies loop healing as the minimal graph-native CP location, while keeping the microscopic origin and loop parameters open.
