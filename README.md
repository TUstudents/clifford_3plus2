...
- `qca_smv0` is a focused simulator sidecar for the next SM-QCA prototype; it currently implements the free BCC Weyl/Dirac bulk walk, static SM gauge-background transport, pure dynamic SM gauge fields, a local Higgs/Yukawa collision, and a finite-path FN recirculation module, but should not be read as a boundary, matter-backreaction, dynamic-Higgs, or flavor-derivation claim.
