...
- `cusp` implements the first CUSP-PLAN target certificates: the `(2,3)`
  ...
  cusp-module amplitude measure plus finite center-topology selection, pending
  microscopic BCC/SM derivation of that topology;
  the CP witness now survives one fixed non-unit positive path-amplitude
  deformation, but the physical coefficient measure is still not derived.
