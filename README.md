...
| [`qca_smv0`](src/clifford_3plus2_d5/qca_smv0/) | **Stage 33 PASS** | Focused simulator sidecar for the next Standard-Model QCA prototype. It now includes the free BCC Weyl/Dirac bulk walk, static and dynamic SM gauge transport, Higgs/Yukawa collisions, FN recirculation, center-holonomy CP inputs, three-family production ticks, physical-right bridged transport/current/source updates, rollout/Gauss/energy/variational monitors, refinement and adjoint limitation audits, an explicit inverse helper for the current physical-right production tick, multi-step trajectory reversibility, Loschmidt/tangent echo diagnostics, a local echo Gram audit, and echo-Gram scale stability. Boundary, quantized registers, microscopic bridge/input derivations, exact energy conservation, and timestep-convergence claims remain future sessions. |
