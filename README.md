...
| [`qca_smv0`](src/clifford_3plus2_d5/qca_smv0/) | **Stage 2 PASS** | Focused simulator sidecar for the next Standard-Model QCA prototype. Stage 1 implements the free BCC Weyl/Dirac bulk walk in JAX. Stage 2 adds a local 32-component SM carrier, static `SU(3)_c x SU(2)_L x U(1)_Y` BCC gauge links, gauge-covariant Dirac transport, Wilson-loop response, weak-link covariant-derivative scaling, and JIT compatibility. Boundary, dynamic gauge fields, Higgs/Yukawa, and flavor recirculation remain future sessions. |
...
- `qca_smv0` is a focused simulator sidecar for the next SM-QCA prototype; it currently implements the free BCC Weyl/Dirac bulk walk and static SM gauge-background transport, but should not be read as a boundary, dynamic gauge, Higgs, or flavor claim.
