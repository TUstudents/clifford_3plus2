...
- `qca_smv0` is a focused simulator sidecar for the next SM-QCA prototype; it currently implements the free BCC Weyl/Dirac bulk walk, static SM gauge-background transport, pure dynamic SM gauge fields, one-generation and three-family local Higgs/Yukawa collisions, finite-path FN recirculation, and center-holonomy CP coefficients, but should not be read as a boundary, matter-backreaction, dynamic-Higgs, or flavor-derivation claim.
