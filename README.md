...
- `qca_smv0` is a focused simulator sidecar for the next SM-QCA prototype; it currently implements the BCC/SM/Higgs/FN/production stack through an explicit inverse helper, multi-step reversibility audit, Loschmidt/tangent echo diagnostics, local echo Gram audit, echo-Gram scale-stability audit, finite all-sector cone audit, a finite momentum-only Gauss relaxation solver, and a Gauss-projected wrapper for the current physical-right production tick, but should not be read as a boundary, quantized-register, microscopic bridge, full nonlinear Gauss projection, exact full-energy conservation, timestep-refined energy convergence, energy-convergent reversible-integrator, continuum causal-cone, or flavor/Higgs-parameter derivation claim.
