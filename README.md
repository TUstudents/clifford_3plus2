...
- `qca_smv0` is a focused simulator sidecar for the next SM-QCA prototype; it currently implements the free BCC Weyl/Dirac bulk walk, static SM gauge-background transport, pure dynamic SM gauge fields, one-generation and three-family local Higgs/Yukawa collisions, finite-path FN recirculation, center-holonomy CP coefficients, dynamic classical Higgs-field evolution, and Higgs gauge-current backreaction, but should not be read as a boundary, fermion-backreaction, quantized-scalar, full sourced SM gauge-evolution, or flavor/Higgs-parameter derivation claim.
