...
| [`qca_smv0`](src/clifford_3plus2_d5/qca_smv0/) | **Stage 49 PASS** | Focused simulator sidecar for the next Standard-Model QCA prototype. It now includes the free BCC Weyl/Dirac bulk walk, static and dynamic SM gauge transport, Higgs/Yukawa collisions, compressed effective FN Yukawa production rollouts, exact hidden-network FN recirculation as a reference dilation, center-holonomy CP inputs, three-family production ticks, physical-right bridged transport/current/source updates, rollout/Gauss/energy/variational monitors, refinement and adjoint limitation audits, an explicit inverse helper for the current physical-right production tick, multi-step trajectory reversibility, Loschmidt/tangent echo diagnostics, a local echo Gram audit, echo-Gram scale stability, a finite-horizon echo-spectrum audit, a finite-stencil locality audit, a dense-workload scaling audit, local analytic Wilson and physical-right fermion-current replacements, a local-force production-rollout smoke test, a multi-step local-force recorded-rollout audit, a local-force production profiling certificate, a numerical local-force spatial-support audit, a one-tick full-production spatial-support audit, a three-tick family-cone audit, a two-tick all-sector cone audit, a finite momentum-only Gauss relaxation solver, an explicit Gauss-projected production-step wrapper, and a finite projected-rollout audit. Boundary, quantized registers, microscopic bridge/input derivations, full nonlinear Gauss-preserving integration, exact energy conservation, timestep-convergence claims, continuum Lyapunov claims, and large-lattice spatial echo measurements remain future sessions. |
...
- `qca_smv0` is a focused simulator sidecar for the next SM-QCA prototype; it currently implements the BCC/SM/Higgs/FN/production stack through an explicit inverse helper, multi-step reversibility audit, Loschmidt/tangent echo diagnostics, local echo Gram audit, echo-Gram scale-stability audit, finite all-sector cone audit, a finite momentum-only Gauss relaxation solver, a Gauss-projected wrapper for the current physical-right production tick, and a finite projected-rollout audit, but should not be read as a boundary, quantized-register, microscopic bridge, full nonlinear Gauss projection, exact full-energy conservation, timestep-refined energy convergence, energy-convergent reversible-integrator, continuum causal-cone, or flavor/Higgs-parameter derivation claim.
