...
- `qca_smv0` is a focused simulator sidecar for the next SM-QCA prototype; it currently implements the BCC/SM/Higgs/FN/production stack through an explicit inverse helper, multi-step reversibility audit, and Loschmidt echo diagnostic for the current physical-right production tick, but should not be read as a boundary, quantized-register, microscopic bridge, Gauss-projection, exact full-energy conservation, timestep-refined energy convergence, energy-convergent reversible-integrator, or flavor/Higgs-parameter derivation claim.
