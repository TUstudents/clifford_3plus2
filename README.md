...
- `qca_smv0` is a focused simulator sidecar for the next SM-QCA prototype; it currently implements the free BCC Weyl/Dirac bulk walk, static SM gauge-background transport, pure dynamic SM gauge fields, one-generation and three-family local Higgs/Yukawa collisions, finite-path FN recirculation, center-holonomy CP coefficients, dynamic classical Higgs-field evolution, Higgs gauge-current backreaction, local Yukawa fermion backreaction into Higgs momenta, family-summed BCC streaming currents, family-sourced SM gauge ticks, and the full family production tick, but should not be read as a boundary, quantized-register, unified gauge-convention, or flavor/Higgs-parameter derivation claim.
